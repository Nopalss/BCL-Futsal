...
class Booking extends CI_Controller
{
    ...
    public function tambahBooking()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['jam_booking'] = $this->modelfutsal->get_jam_booking($this->session->userdata('tanggal'), $this->session->userdata('id_lapangan'));
            ...
            if ($this->form_validation->run() == false) {
                ...
                if($pecah[0] >= date('Y') && $pecah[1] >= date('m')){
                    if($pecah[1] == date('m') && $pecah[2] >= date('d')){
                        ...
                        $this->session->set_flashdata('message', '<div class="alert alert-message alert-danger" role="alert"><i class="fas fa-info-circle"></i> Tanggal Tidak Boleh Kurang Dengan Tanggal Sekarang !</div>');
                        redirect('booking/editTanggal/'. $id);
                    }
                    $tanggal1 = [
                        'tanggal' => $tanggal,
                        'id_lapangan' => $id_lapangan,
                    ...
                    $this->session->set_userdata($lapangan2);
                    $data['pelanggan'] = $this->modelfutsal->getById('pelanggan', ['id' => $data['booking']['id_pelanggan']]);
                    $data['jam_booking'] = $this->modelfutsal->get_jam_booking($this->session->userdata('tanggal'), $this->session->userdata('id_lapangan'));
                    $this->load->view('templates/header', $data);
                    $this->load->view('templates/sidebar', $data);
                    $this->load->view('templates/topbar', $data);
    ...
    public function editBooking($id){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($this->modelfutsal->seleksiJ($jam, $tanggal) == true){
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-success alert-message mb-0" role="alert"><i class="fas fa-info-circle"></i> Data Booking berhasil diUpdate!</div>');
                redirect('booking');
            }else{
                $this->session->set_flashdata('message', '<div class="alert alert-danger alert-message" role="alert"><i class="fas fa-info-circle"></i> Jam yang anda pilih sudah lewat, silahkan pilih lagi!</div>');
                redirect('booking/editBooking/').$id;
            }
        }
    }
    public function printBooking($id){
        $data['booking'] = $this->modelfutsal->getBooking($id);
        $data['id_booking'] = $id;
        $this->load->view('booking/printBooking', $data);
    }
}